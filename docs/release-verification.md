...
- `*.tar.gz`: release binaries for `lattice-node`, `lattice-mining-coordinator`,
  `lattice-miner`, and `lattice-proof-verifier`.
...
`lattice-mining-coordinator`, `lattice-miner`, and `lattice-proof-verifier`.
After archive
...
and verifies the same persisted tip; it also confirms the verifier executable
starts successfully.
