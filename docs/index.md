...
- [design/source-agnostic-sync.md](design/source-agnostic-sync.md) — the sync-path realization of proof-availability: peers are interchangeable byte-providers, rotate past empty/lying peers (penalize the peer, never blacklist the honest tip), fail closed only when no peer serves a valid proof. Follows established multi-peer sync practice (Bitcoin headers-first, go-ethereum snap, IPFS/Bitswap, AuxPoW, data-availability-vs-validity).
- [design/bulk-header-content-addressing.md](design/bulk-header-content-addressing.md) — the per-header CID-validation invariant the bulk sync path enforces.
- [design/block-content-storage.md](design/block-content-storage.md) — the gossiped-block resolution and storage invariant.
- [design/storage-layer-state-retention.md](design/storage-layer-state-retention.md) — storage-layer retained roots for state retention, with its load-bearing requirements.
- [analysis/merged-mining-incentives.md](analysis/merged-mining-incentives.md) — merged-mining incentive & centralization analysis on the per-chain-independent / opt-in model.
