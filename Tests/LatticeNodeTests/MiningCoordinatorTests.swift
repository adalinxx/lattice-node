...
private actor StubNodeClient: MiningCoordinatorNodeClient {
    ...
    private(set) var submissions: [(workId: String, nonce: UInt64, hash: String?)] = []

    init(
        workResponses: [MiningCoordinatorWork?],
        submission: MiningSolutionSubmission = .init(accepted: true, status: "accepted", blockHash: "b", height: 1),
    ...
    func submit(workId: String, nonce: UInt64, hash: String?) async throws -> MiningSolutionSubmission {
        submissions.append((workId, nonce, hash))
...
private actor RetryingSubmitStub: MiningCoordinatorNodeClient {
    ...
    func submit(workId: String, nonce: UInt64, hash: String?) async throws -> MiningSolutionSubmission {
        ...
        return MiningSolutionSubmission(accepted: true, status: "accepted", blockHash: "b", height: 1)
    }

    func recordedSubmissions() -> [(workId: String, nonce: UInt64, hash: String?)] {
        submissions
    }
}
...
final class MiningCoordinatorTests: XCTestCase {
    ...
    func testWorkAdoptsNodeStaleTokenAndFallsBackToWorkIdForLegacyNodes() {
        // The node returns a stable staleToken (tip hash) distinct from the
        // volatile workId (candidate CID, restamped every fetch). The work must
        // adopt it.
        let withToken = MiningCoordinatorWork(template: TemplateResponse(
            workId: "candidate-cid", blockHex: "00", effectiveTarget: "ff", staleToken: "tip-hash"
        ))
        XCTAssertEqual(withToken?.workId, "candidate-cid")
        XCTAssertEqual(withToken?.staleToken, "tip-hash")

        // Older node that doesn't emit staleToken: fall back to workId so the
        // probe keeps working (the pre-staleToken behavior).
        let legacy = MiningCoordinatorWork(template: TemplateResponse(
            workId: "candidate-cid", blockHex: "00", effectiveTarget: "ff"
        ))
        XCTAssertEqual(legacy?.staleToken, "candidate-cid")
    }
    ...
    func testHTTPSubmitDecodesProtocolRejectionDespiteHTTPConflict() throws {
        let response = try XCTUnwrap(HTTPURLResponse(
            url: URL(string: "http://127.0.0.1/chain/submit-work")!,
            statusCode: 409,
            httpVersion: nil,
            headerFields: nil
        ))
        let body = Data(#"{"accepted":false,"status":"duplicate","blockHash":null,"height":null}"#.utf8)
        ...
        XCTAssertEqual(submission.status, "duplicate")
    }
    ...
    func testFirstValidWorkerResultWinsAndSubmitsOnce() async {
        ...
        let fast = MiningCoordinatorWorker(id: "fast") { work, _ in
            MiningWorkerResult(workerId: "fast", workId: work.workId, nonce: 7, hash: "abc")
        }
        let node = StubNodeClient(workResponses: [work, work])
        ...
        XCTAssertEqual(submissions.first?.hash, "abc")
    ...
    func testRejectedSubmissionDoesNotLoopDuplicateSubmitsAcrossBatches() async {
        ...
        let node = StubNodeClient(
            ...
            submission: .init(accepted: false, status: "wrongTarget")
        )
        let coordinator = MiningCoordinator(
            nodeClient: node,
            workers: workers,
        ...
        XCTAssertEqual(firstSubmission.status, "wrongTarget")
        XCTAssertEqual(secondSubmission.status, "wrongTarget")
        let submissions = await node.submissions
    ...
    func testTransientSubmitFailureResubmitsSameSolution() async {
        let worker = MiningCoordinatorWorker(id: "fast") { work, _ in
            MiningWorkerResult(workerId: "fast", workId: work.workId, nonce: 99, hash: "deadbeef")
        }
        ...
        XCTAssertTrue(submissions.allSatisfy { $0.workId == "work-1" && $0.nonce == 99 && $0.hash == "deadbeef" },
                      "every retry must re-POST the SAME solved block bytes")
        let metrics = await coordinator.metrics()
        XCTAssertEqual(metrics.acceptedSolutionCount, 1)
        XCTAssertEqual(metrics.rejectedSolutionCount, 0)
    }
    ...
    func testCleanRejectionIsNotRetried() async {
        ...
        XCTAssertFalse(submission.accepted)
        XCTAssertEqual(submission.status, "wrongTarget")
        let submissionCount = await node.submissionCount()
    ...
    func testHTTPFetchWorkTreats503AsTransientBackoffNotFatal() async throws {
        StubTemplateURLProtocol.responder = { _ in
            (503, Data(#"{"error":"No parent-state-continuous child candidate"}"#.utf8))
        }
    ...
    func testHTTPFetchStaleTokenUsesChainInfoTipWithoutTemplateFetch() async throws {
        StubTemplateURLProtocol.responder = { request in
            guard request.url?.path == "/api/chain/info" else {
